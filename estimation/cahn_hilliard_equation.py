...
# ## Time simulation
# 
# The `tsme` package provides a number of pre-implemented dynamical systems, so we simply import the class
# `CahnHilliard`. (See this [page]
# (https://nonlinear-physics.zivgitlabpages.uni-muenster.de/ag-kamps/tsme/source/tsme.premade_models.html)
# for more models.)
...
with open("tsme_examples/data/PDEs/che.json", "r") as file:
    data = rick.load(file)
...
# We need to pass the trajectory, domain size and the time stamps to our model. Then we can automatically generate a
# library of possible test functions for our right-hand side. Here we take the powers of $u$ up to third order and then
# all spatial derivatives upto fourth order of said powers (these include both spatial directions and their mixed
# derivatives).
...
with open("tsme_examples/data/estimates/che" + f"_{ode_order}_{pde_order}_{kind}_n{norm}_OT.json", "w") as file:
...
estimated_model.print_library_to_file(f'tsme_examples/data/estimates/che_{ode_order}_{pde_order}.txt')
...
with open(f"tsme_examples/data/estimates/che_{ode_order}_{pde_order}.json", "w") as file:
...
animate(sol_est, time=t, savepath=f"tsme_examples/diagrams/PDEs/che_{ode_order}_{pde_order}.mp4")
